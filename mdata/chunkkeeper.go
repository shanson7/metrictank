...
// ChunkKeeper takes in new values, updates the in-memory data and streams the points to aggregators
...
// ChunkKeeper is concurrency-safe
type ChunkKeeper struct {
	sync.RWMutex
...
// NewChunkKeeper creates a metric with given key, it retains the given number of chunks each chunkSpan seconds long
...
func NewChunkKeeper(parent *AggMetric, store Store, cachePusher cache.CachePusher, key string, ret conf.Retention, dropFirstChunk bool) *ChunkKeeper {
	ck := ChunkKeeper{
...
func (a *ChunkKeeper) SyncChunkSaveState(ts uint32) {
	a.Lock()
	defer a.Unlock()
...
func (a *ChunkKeeper) getChunk(pos int) *chunk.Chunk {
	if pos < 0 || pos >= len(a.Chunks) {
		panic(fmt.Sprintf("ChunkKeeper %s queried for chunk %d out of %d chunks", a.Key, pos, len(a.Chunks)))
	}
	return a.Chunks[pos]
}
...
func (a *ChunkKeeper) Get(from, to uint32) (uint32, []chunk.Iter) {
	pre := time.Now()
	if LogLevel < 2 {
...
func (a *ChunkKeeper) pushToCache(c *chunk.Chunk) {
	// push into cache
...
func (a *ChunkKeeper) persist(pos int) {
	chunk := a.Chunks[pos]
	pre := time.Now()
...
func (a *ChunkKeeper) Add(ts uint32, val float64) {
	a.Lock()
	defer a.Unlock()
...
func (a *ChunkKeeper) GC(chunkMinTs, metricMinTs uint32) bool {
	a.Lock()
	defer a.Unlock()

	// this ChunkKeeper has never had metrics written to it.
	if len(a.Chunks) == 0 {
		return true
	}
